(async () => {
  // Create overlay
  const overlay = document.createElement('div');
  overlay.style.cssText = `
    position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
    background: black; color: red; font-family: monospace;
    z-index: 9999999; display: flex; flex-direction: column;
    align-items: center; justify-content: center; overflow: hidden;
    filter: contrast(2) brightness(0.8);
  `;
  document.body.appendChild(overlay);

  // Add VHS scanlines
  const scanlines = document.createElement('div');
  scanlines.style.cssText = `
    position: absolute; width: 100%; height: 100%;
    background: repeating-linear-gradient(black 0px, rgba(255, 0, 0, 0.05) 1px, black 2px);
    animation: flicker 0.1s infinite;
    z-index: 99999999;
    pointer-events: none;
  `;
  overlay.appendChild(scanlines);

  // Add creepy image
  const img = document.createElement('img');
  img.src = "data:image/jpeg;base64,";
  img.style.cssText = `
    max-width: 400px;
    filter: drop-shadow(0 0 10px red) brightness(1.5) contrast(1.2);
    margin-bottom: 20px;
  `;
  overlay.appendChild(img);

  // Whisper audio
  const audio = new Audio('https://cdn.pixabay.com/download/audio/2023/03/04/audio_b2bce1fd24.mp3?filename=creepy-whispers-138092.mp3');
  audio.volume = 0.8;
  audio.play().catch(() => {}); // Safe play

  // IP fetch & typing
  const msg = document.createElement('div');
  msg.style.fontSize = '2em';
  overlay.appendChild(msg);

  let ip = "0.0.0.0";
  try {
    const res = await fetch("https://api.ipify.org?format=json");
    const data = await res.json();
    ip = data.ip;
  } catch {}

  const fullText = `I'm watching you...\nIP: ${ip}`;
  let i = 0;
  function type() {
    if (i < fullText.length) {
      msg.textContent += fullText[i++];
      setTimeout(type, 100);
    }
  }
  type();

  // After delay, start "meltdown"
  setTimeout(() => {
    audio.pause();
    overlay.innerHTML = '';
    overlay.style.background = 'radial-gradient(red, black)';
    overlay.style.animation = 'glitch 0.05s infinite';
    const chaos = document.createElement('div');
    chaos.textContent = "🔻 SYSTEM FAILURE 🔻";
    chaos.style.cssText = `
      font-size: 4em; color: white; text-shadow: 0 0 10px red;
      animation: flicker 0.1s infinite alternate;
    `;
    overlay.appendChild(chaos);
  }, 9000);

  // Keyframes (add to page)
  const style = document.createElement('style');
  style.innerHTML = `
    @keyframes flicker {
      0% { opacity: 1; } 50% { opacity: 0.7; } 100% { opacity: 1; }
    }
    @keyframes glitch {
      0% { transform: translate(0, 0) scale(1); filter: hue-rotate(0deg); }
      25% { transform: translate(-5px, 2px) scale(1.02); filter: hue-rotate(90deg); }
      50% { transform: translate(5px, -2px) scale(0.98); filter: hue-rotate(180deg); }
      75% { transform: translate(-3px, 1px) scale(1.01); filter: hue-rotate(270deg); }
      100% { transform: translate(0, 0) scale(1); filter: hue-rotate(360deg); }
    }
  `;
  document.head.appendChild(style);
})();
